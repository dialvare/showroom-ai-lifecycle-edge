 ...
 ** Check the *Make deployed models available through an external route* box to make the model externally available.
 ** Uncheck the *Token authentication* box.
 ...
 ** *Path*: Indicate the path to your model location in the S3 bucket. That's: `models/serving/stress-detection/`. We didn't specify the full path to the model because KServe will automatically pull it from the _/1/_ subdirectory in the path specified.
